...
namespace FarmaciaCarolAPI.Controllers
{
    ...
    public class FacturasController : ControllerBase
    {
        ...
        {
            if (string.IsNullOrEmpty(model.Estado) || (model.Estado != "Contado" && model.Estado != "Crédito"))
                return BadRequest("El estado debe ser 'Contado' o 'Crédito'.");

            if (model.Total < 0)
                return BadRequest("No se permiten montos negativos.");
